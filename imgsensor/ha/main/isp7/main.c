...
static SENINF_RETURN seninf_checkpipe(int chunk_hsfhandle)
{
    SENINF_LOGE("[%s] +\n", __func__);
    ...
    ret = seninf_auth(&preg_va, chunk_hsfhandle);
...
static int imgsensor_ha_handler (int session, int command, uint32_t paramTypes, MTEEC_PARAM *param)
{
    ...
    SENINF_LOGE("[%s] session=0x%x, command=0x%x, paramTypes=0x%x, TG status:%d \n", __func__, session, command, paramTypes, param[0].value.b);

    switch (command) {
        case SENINF_TEE_CMD_CHECKPIPE:
            ret = seninf_checkpipe(param[0].value.b);
            break;
